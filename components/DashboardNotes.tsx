...
export function DashboardNotes() {
  ...
  // 2. SOHBET DEĞİŞİNCE MESAJLARI GETİR
  useEffect(() => {
  ...
  // 🚀 3. ULTRA MEGA ANLIK DİNLEYİCİ (GELİŞTİRİLMİŞ)
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        async (payload) => {
          console.log("Anlık Değişiklik Yakalandı:", payload)

          // 🆕 YENİ MESAJ GELDİYSE VEYA GÖNDERİLDİYSE
          if (payload.eventType === 'INSERT') {
            const newMessage = payload.new;

            // Eğer mesaj şu an açık olan sohbete aitse listeye ekle
            const isRelevant = 
                (activeChat === "genel" && !newMessage.receiver_id) ||
                (activeChat === newMessage.user_id && newMessage.receiver_id === currentUser.id) ||
                (newMessage.user_id === currentUser.id && newMessage.receiver_id === activeChat);

            if (isRelevant) {
                // İsmi ve detayları getirmek için tek bir hızlı fetch yap (Joins için)
                const { data: enrichedNote } = await supabase
                    .from('notes')
                    .select(`*, profiles (first_name, last_name, department), projects (project_code), reply_to:reply_to_id ( content, profiles (first_name, last_name) )`)
                    .eq('id', newMessage.id)
                    .single();

                if (enrichedNote) {
                    setNotes(prev => [...prev, enrichedNote]);
                    if (newMessage.receiver_id === currentUser.id) markAsRead(activeChat!);
                }
            }
          } 
          ...
          // 😄 REAKSİYON GÜNCELLENDİYSE
          else if (payload.eventType === 'UPDATE') {
             fetchNotes(); // Reaksiyonlar kompleks olduğu için hızlıca yenile
          }

          // Bildirim sayılarını her türlü güncelle
          loadUnreadCounts();
        }
      )
      .subscribe();
  ...
  const markAsRead = async (chatId: string) => {
    if (!currentUser || chatId === "genel") return;
  ...
  const addNote = async () => {
    ...
    if (!error) { setNewNote(""); setSelectedProjectId(""); setReplyTo(null); }
    setLoading(false)
  }
  ...
  return (
    <div className="flex flex-col h-[600px] font-sans bg-white border border-slate-200 rounded-[1.5rem] shadow-sm overflow-hidden w-full relative">
      {activeView === "list" ? (
          <div className="flex flex-col h-full bg-slate-50">
              ...
              <div className="flex-1 overflow-y-auto p-2 space-y-1">
                  <button onClick={() => { setActiveChat("genel"); setActiveView("chat"); }} className="w-full flex items-center gap-3 p-3 rounded-2xl hover:bg-slate-200/50 transition-all">
                      <div className="h-12 w-12 bg-emerald-500 rounded-full flex items-center justify-center text-white shadow-inner"><Users className="h-6 w-6" /></div>
                  ...
                  {users.filter(u => u.id !== currentUser?.id).map((u) => (
                      <button key={u.id} onClick={() => { setActiveChat(u.id); setActiveView("chat"); }} className="w-full flex items-center gap-3 p-3 rounded-2xl hover:bg-slate-200/50 transition-all relative">
                          <div className="h-12 w-12 bg-slate-800 rounded-full flex items-center justify-center text-white font-black text-sm">{u.first_name?.charAt(0)}{u.last_name?.charAt(0)}</div>
                          <div className="flex flex-col items-start overflow-hidden flex-1"><span className="font-bold text-slate-800 text-sm">{u.first_name} {u.last_name}</span><span className="text-[10px] text-slate-500">{u.department}</span></div>
                          {unreadCounts[u.id] > 0 && <span className="bg-rose-500 text-white text-[10px] font-black px-2 py-0.5 rounded-full absolute right-4 top-1/2 -translate-y-1/2">{unreadCounts[u.id]}</span>}
                      </button>
                  ))}
              </div>
          </div>
          ...
          <div className="flex flex-col h-full relative">
              <div className="flex items-center gap-3 p-3 bg-white border-b border-slate-200 shadow-sm z-10">
                  ...
                  <div className={`h-10 w-10 rounded-full flex items-center justify-center text-white font-black text-xs ${activeChat === "genel" ? 'bg-emerald-500' : 'bg-slate-800'}`}>{activeChat === "genel" ? <Users className="h-5 w-5" /> : `${activeChatUser?.first_name?.charAt(0)}${activeChatUser?.last_name?.charAt(0)}`}</div>
                  <div className="flex flex-col"><h2 className="text-sm font-black text-slate-800">{activeChat === "genel" ? 'Genel Pano' : `${activeChatUser?.first_name} ${activeChatUser?.last_name}`}</h2><p className="text-[9px] font-bold text-emerald-600">{activeChat === "genel" ? 'Tüm Şirket' : activeChatUser?.department}</p></div>
              </div>
              <div ref={scrollRef} className="flex-1 overflow-y-auto p-3 space-y-3 bg-[#e5ddd5]/30" style={{backgroundImage: `url('https://i.pinimg.com/1200x/8c/98/99/8c98994518b575bfd8c949e91d20548b.jpg')`, backgroundBlendMode: 'soft-light'}}>
                  {notes.map((note) => {
              ...
              <div className="p-2 bg-[#f0f2f5] border-t border-slate-200 shrink-0 relative">
                  ...
                  <div className="flex flex-col gap-1.5 px-1 relative z-20">
                      <div className="flex bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm h-8">
                          ...
                          <select className="flex-1 bg-transparent text-[10px] font-bold text-slate-600 px-2 outline-none" value={selectedProjectId} onChange={(e) => setSelectedProjectId(e.target.value)}>
                              <option value="">-- Proje Seç --</option>
                              {projects.map(p => <option key={p.id} value={p.id}>{p.project_code}</option>)}
                          </select>
                      </div>
                      <div className="flex items-end gap-2">
                          <Textarea placeholder="Mesaj yazın..." className="min-h-[40px] max-h-[80px] resize-none bg-white border-none rounded-xl text-xs font-medium focus-visible:ring-0 p-2.5 shadow-sm" value={newNote} onChange={(e) => setNewNote(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); addNote(); } }} />
                          <Button onClick={addNote} disabled={loading || !newNote.trim()} className="h-10 w-10 rounded-full bg-emerald-500 hover:bg-emerald-600 text-white shadow-md active:scale-95 p-0 flex items-center justify-center">{loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4 ml-0.5" />}</Button>
                      </div>
                  </div>
              </div>
          </div>
