    ...
    <div className="flex flex-col gap-8 p-2 max-w-[1600px] mx-auto w-full font-sans">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 bg-white/60 backdrop-blur-2xl border border-white/50 p-6 rounded-[2rem] shadow-sm">
        <div className="flex items-center gap-5">
            ...
            <div>
                <h1 className="text-3xl font-black tracking-tight text-slate-900">Eksik Malzeme Radarı</h1>
                <p className="text-slate-500 font-medium text-sm mt-1">Saha ve üretim hattından gelen anlık acil parça talepleri.</p>
            </div>
        </div>
        <div className="relative w-full md:w-72">
            ...
            <Input 
                ...
                className="pl-11 h-14 bg-white/80 border-white text-sm font-medium focus:ring-2 focus:ring-red-500 shadow-sm rounded-2xl transition-all w-full"
            />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        ...
        {filteredItems.map((item) => {
            ...
            <div 
                key={item.id} 
                className={`relative flex flex-col justify-between bg-white/70 backdrop-blur-xl border-2 rounded-[2rem] p-6 transition-all duration-500 hover:-translate-y-1 hover:shadow-xl overflow-hidden group
            ...
            >
                ...
                <div className="flex items-start justify-between mb-6 relative z-10">
                    <div className="flex flex-col gap-2">
                        <div className="flex items-center gap-2">
                            <span className={`text-[10px] font-black uppercase tracking-widest px-2 py-1 rounded-md w-max ${isUrgent ? 'bg-red-100 text-red-600' : 'bg-emerald-100 text-emerald-700'}`}>
                        ...
                        <div className="flex items-center gap-2 mt-1">
                            <span className="font-mono font-bold text-slate-500 text-[10px] bg-slate-200/50 px-2 py-0.5 rounded border border-slate-200">
                                PRJ: {item.projects?.project_code || "KOD-YOK"}
                            </span>
                            <span className="font-bold text-slate-700 text-sm truncate max-w-[150px]">{item.projects?.customers?.name || "Bağımsız Talep"}</span>
                        </div>
                    </div>
                ...
                <div className="flex flex-col gap-1 mb-8 relative z-10">
                    ...
                    <div className="flex items-end gap-3">
                        ...
                        <span className="text-xl font-black text-blue-600 bg-blue-50 px-3 py-1 rounded-xl mb-0.5 border border-blue-100">
                            x{item.quantity}
                        </span>
                    </div>
                </div>
                ...
                <div className="mt-auto pt-4 border-t border-slate-100 relative z-10 flex flex-col gap-4">
                    ...
                    {isUrgent ? (
                        <Button 
                            ...
                            className="w-full h-14 rounded-2xl bg-slate-900 hover:bg-slate-800 text-white font-bold shadow-lg shadow-slate-900/20 transition-all flex items-center justify-between px-6 group/btn mt-2"
                        >
                            <span className="flex items-center gap-2">
                                {actionLoading === item.id ? <Loader2 className="h-5 w-5 animate-spin" /> : <ShoppingCart className="h-5 w-5 text-red-400" />}
                                {actionLoading === item.id ? "İşleniyor..." : "Satın Almaya Aktar"}
                            </span>
                            <ArrowRight className="h-5 w-5 text-slate-500 group-hover/btn:text-white group-hover/btn:translate-x-1 transition-all" />
                        </Button>
                    ) : (
                        <div className="w-full h-14 mt-2 rounded-2xl bg-emerald-50 border border-emerald-100 flex items-center justify-center text-emerald-700 font-bold gap-2">
        ...
        {filteredItems.length === 0 && !loading && (
            <div className="col-span-full flex flex-col items-center justify-center p-20 bg-white/50 backdrop-blur-md rounded-[3rem] border-2 border-dashed border-slate-200">
                <div className="bg-emerald-100 p-6 rounded-full mb-6"><Factory className="h-16 w-16 text-emerald-600" /></div>
                <h2 className="text-3xl font-black text-slate-800">Üretim Hattı Güvende</h2>
                <p className="text-lg font-medium text-slate-500 mt-2 text-center max-w-md">Sahadan gelen herhangi bir eksik malzeme talebi bulunmuyor.</p>
            </div>
        )}
      </div>
    </div>
