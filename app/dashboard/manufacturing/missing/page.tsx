    ...
    <div className="flex flex-col gap-6 md:gap-8 font-sans max-w-[1600px] mx-auto w-full pb-10">
      ...
      <div className="flex flex-col xl:flex-row items-start xl:items-center justify-between gap-4 md:gap-6 bg-card/60 backdrop-blur-2xl border border-border/50 p-5 md:p-6 lg:p-8 rounded-[1.5rem] md:rounded-[2.5rem] shadow-sm">
        <div className="flex items-center gap-4 md:gap-5 w-full xl:w-auto">
            <div className="bg-gradient-to-br from-red-500 to-rose-600 p-3 md:p-4 rounded-2xl shadow-lg shadow-red-500/30 shrink-0">
                <AlertCircle className="h-6 w-6 md:h-8 md:w-8 text-primary-foreground" />
            </div>
            <div>
                <h1 className="text-xl md:text-3xl font-black tracking-tight text-foreground">Eksik Malzeme Radarı</h1>
                <p className="text-muted-foreground font-medium text-xs md:text-sm mt-1">Saha ve üretim hattından gelen anlık acil parça talepleri.</p>
            </div>
        </div>
        <div className="relative w-full xl:w-80 shrink-0">
            ...
            <Input 
                ...
                className="pl-11 md:pl-12 h-12 md:h-14 bg-card/80 border-white text-xs md:text-sm font-medium focus:ring-2 focus:ring-red-500 shadow-sm rounded-xl md:rounded-2xl transition-all w-full"
            />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 md:gap-6">
        ...
        {filteredItems.map((item) => {
            ...
            <div 
                key={item.id} 
                className={`relative flex flex-col justify-between bg-card/70 backdrop-blur-xl border-2 rounded-[1.5rem] md:rounded-[2rem] p-5 md:p-6 transition-all duration-500 hover:-translate-y-1 hover:shadow-xl overflow-hidden group
            ...
            >
                ...
                <div className="flex flex-col gap-3 md:gap-0 md:flex-row md:items-start justify-between mb-5 md:mb-6 relative z-10">
                    <div className="flex flex-col gap-1.5 md:gap-2">
                        <div className="flex items-center gap-2 flex-wrap">
                            ...
                            <span className="text-[9px] md:text-[10px] font-black uppercase tracking-widest px-2 py-1 rounded-md bg-muted text-muted-foreground">
                                {formatTalepNumber(item.id)}
                            </span>
                        </div>
                        <div className="flex flex-wrap md:flex-nowrap items-center gap-1.5 md:gap-2 mt-1">
                            <span className="font-mono font-bold text-muted-foreground text-[9px] md:text-[10px] bg-slate-200/50 px-2 py-0.5 rounded border border-border">
                ...
                <div className="flex flex-col gap-1 mb-6 md:mb-8 relative z-10">
                    <span className="text-[10px] md:text-xs font-bold text-muted-foreground uppercase tracking-widest">İstenen Malzeme</span>
                    <div className="flex items-end gap-2 md:gap-3">
                        <h3 className="text-xl md:text-2xl font-black text-foreground leading-tight truncate">
                            {item.product_name}
                        </h3>
                        <span className="text-lg md:text-xl font-black text-primary bg-primary/10 px-2 md:px-3 py-1 rounded-xl mb-0.5 md:mb-1 border border-blue-100 shrink-0">
                            x{item.quantity}
                        </span>
                    </div>
                </div>

                <div className="mt-auto pt-4 border-t border-border relative z-10 flex flex-col gap-4">
                    ...
                    {isUrgent ? (
                        <Button 
                            ...
                            className="w-full h-12 md:h-14 rounded-xl md:rounded-2xl bg-slate-900 hover:bg-slate-800 text-primary-foreground font-bold shadow-lg shadow-slate-900/20 transition-all flex items-center justify-between px-4 md:px-6 group/btn mt-2 text-xs md:text-sm"
                        >
                            ...
                            <ArrowRight className="h-4 w-4 md:h-5 md:w-5 text-muted-foreground group-hover/btn:text-primary-foreground group-hover/btn:translate-x-1 transition-all" />
                        </Button>
                    ) : (
        ...
        {filteredItems.length === 0 && !loading && (
            <div className="col-span-full flex flex-col items-center justify-center p-12 md:p-20 bg-card/50 backdrop-blur-md rounded-[2rem] md:rounded-[3rem] border-2 border-dashed border-border">
                ...
                <h2 className="text-xl md:text-3xl font-black text-foreground text-center">Üretim Hattı Güvende</h2>
                <p className="text-sm md:text-lg font-medium text-muted-foreground mt-2 text-center max-w-md">Sahadan gelen herhangi bir eksik malzeme talebi bulunmuyor.</p>
            </div>
        )}
      </div>
    </div>
