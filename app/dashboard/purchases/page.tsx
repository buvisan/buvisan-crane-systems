...
export default function PurchasesPage() {
  ...
  // 🚀 YENİ: FORM DÜZENLEME (REVİZE) STATELERİ
  ...
  // 🚀 YENİ: DÜZENLEME MODALINI AÇ
  const openFormEditor = (reqGroup: any) => {
      setEditFormGroup({ ...reqGroup })
      setEditFormItems(JSON.parse(JSON.stringify(reqGroup.items))) // Derin kopya
      setIsFormEditModalOpen(true)
  }

  // 🚀 YENİ: YENİ KALEM EKLE (DÜZENLEME MODU)
  const handleAddNewItemToForm = () => {
      ...
      const newItem = {
          isNew: true, // Veritabanına INSERT atacağımızı belirtmek için
  ...
  // 🚀 YENİ: MEVCUT KALEMİ SİL (DÜZENLEME MODU)
  const handleRemoveItemFromForm = (index: number) => {
      const items = [...editFormItems];
      const itemToDelete = items[index];
      
      if (itemToDelete.id) {
          // Eğer zaten DB'de varsa, silindi olarak işaretle
          itemToDelete.isDeleted = true;
      } else {
          // Eğer yeni eklendiyse, listeden çıkar
          items.splice(index, 1);
      }
      setEditFormItems([...items]);
  }

  // 🚀 YENİ: FORMU KAYDET VE GÜNCELLE
  const handleSaveFormChanges = async () => {
      setIsSavingForm(true);
      try {
          // 1. Ana grup bilgilerini güncelle (Proje No, Cins)
  ...
  return (
    <div className="flex flex-col gap-6 font-sans xl:h-[calc(100vh-100px)] w-full pb-10 xl:pb-0 overflow-hidden">
                                      ...
                                      <div key={reqGroup.request_no} className={`flex flex-col p-4 rounded-xl shadow-sm gap-3 border-2 transition-all ${isUrgent ? 'bg-rose-50/30 border-rose-300 shadow-rose-100' : 'bg-white border-slate-100 hover:border-blue-200'}`}>
                                          ...
                                          <div className="flex flex-col gap-2">
                                              <div className="flex items-center justify-between">
                                                  ...
                                                  {/* 🚀 FORM DÜZENLEME BUTONU */}
                                                  {reqGroup.status === 'BEKLIYOR' && (
                                                      <Button onClick={() => openFormEditor(reqGroup)} variant="outline" size="sm" className="h-8 text-[10px] font-bold text-indigo-600 border-indigo-200 hover:bg-indigo-50"><Edit2 className="h-3.5 w-3.5 mr-1" /> Formu Düzenle</Button>
                                                  )}
                                              </div>
      ...
      <Dialog open={isFormEditModalOpen} onOpenChange={setIsFormEditModalOpen}>
          <DialogContent className="rounded-[2rem] p-6 max-w-4xl border-none shadow-2xl flex flex-col max-h-[90vh]">
              <DialogHeader className="shrink-0 mb-4">
                  <DialogTitle className="text-xl font-black text-slate-800 flex items-center gap-2"><Edit2 className="h-5 w-5 text-indigo-500"/> Formu Revize Et (Düzenle)</DialogTitle>
                  <p className="text-xs font-bold text-slate-500 mt-1">Burada yaptığınız değişiklikler siparişin orijinalini günceller ve her yerde görünür.</p>
              </DialogHeader>

              {editFormGroup && (
                  <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 flex flex-col gap-6">
                      <div className="bg-indigo-50/50 border border-indigo-100 rounded-[1.5rem] p-4 flex flex-col gap-4">
                          <div className="grid grid-cols-3 gap-4">
                              <div className="space-y-2"><Label className="text-[10px] font-bold text-indigo-700 uppercase tracking-widest">Proje No</Label><Input value={editFormGroup.project_code} onChange={e=>setEditFormGroup({...editFormGroup, project_code: e.target.value})} className="font-bold border-indigo-200 focus:ring-indigo-500 h-11 bg-white" /></div>
                              <div className="space-y-2"><Label className="text-[10px] font-bold text-indigo-700 uppercase tracking-widest">Malzeme Cinsi</Label><Input value={editFormGroup.material_type} onChange={e=>setEditFormGroup({...editFormGroup, material_type: e.target.value})} className="font-bold border-indigo-200 focus:ring-indigo-500 h-11 bg-white" /></div>
                              <div className="space-y-2"><Label className="text-[10px] font-bold text-indigo-700 uppercase tracking-widest">Öncelik</Label>
                                  <select value={editFormGroup.priority} onChange={e => setEditFormGroup({...editFormGroup, priority: e.target.value})} className="w-full h-11 px-3 rounded-xl bg-white border border-indigo-200 text-sm font-bold text-slate-700 outline-none focus:ring-2 focus:ring-indigo-500">
                      ...
                      <div className="bg-slate-50 border border-slate-200 rounded-[1.5rem] p-4 flex flex-col gap-4">
                          <Label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-[-5px]">Forma Yeni Kalem Ekle</Label>
                          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                              <div className="space-y-1 col-span-2 md:col-span-3"><Input placeholder="Ürün Tanımı" value={newItemForm.material_name} onChange={e=>setNewItemForm({...newItemForm, material_name: e.target.value})} className="font-bold border-slate-200 h-11 bg-white" /></div>
                              <div className="space-y-1 col-span-1"><Input type="number" placeholder="Stok" value={newItemForm.current_stock} onChange={e=>setNewItemForm({...newItemForm, current_stock: e.target.value})} className="font-bold border-slate-200 h-11 bg-white" /></div>
                              <div className="space-y-1 col-span-1"><Input type="number" placeholder="Miktar" min="1" value={newItemForm.quantity} onChange={e=>setNewItemForm({...newItemForm, quantity: e.target.value})} className="font-black text-blue-600 border-slate-200 h-11 bg-white" /></div>
                          </div>
                          <Button type="button" onClick={handleAddNewItemToForm} className="w-full h-10 bg-slate-800 hover:bg-slate-900 text-white font-bold text-xs rounded-xl"><PlusCircle className="h-4 w-4 mr-2" /> BU SATIRI FORMA EKLE</Button>
                      </div>

                      <div className="border border-slate-200 rounded-2xl overflow-hidden">
                          <table className="w-full text-left text-xs md:text-sm">
                              <thead className="bg-slate-100 border-b border-slate-200 text-slate-500 font-bold"><tr><th className="px-3 py-3">Ürün Tanımı</th><th className="px-3 py-3 text-center w-20">Stok</th><th className="px-3 py-3 text-center w-24">Miktar</th><th className="px-3 py-3 text-right w-16">İşlem</th></tr></thead>
                              <tbody className="divide-y divide-slate-100">
                                  {editFormItems.map((item, index) => {
                                      if (item.isDeleted) return null; // Silinenleri ekranda gösterme
                                      return (
                                      <tr key={index} className={`bg-white hover:bg-slate-50 ${item.isNew ? 'bg-emerald-50/50' : ''}`}>
                                          <td className="px-3 py-2">
                                              <Input value={item.material_name} onChange={e => { const items = [...editFormItems]; items[index].material_name = e.target.value; setEditFormItems(items); }} className="h-9 font-bold text-slate-800 border-transparent hover:border-slate-200 focus:border-blue-500" />
                                          </td>
                                          <td className="px-1 py-2">
                                              <Input type="number" value={item.current_stock} onChange={e => { const items = [...editFormItems]; items[index].current_stock = e.target.value; setEditFormItems(items); }} className="h-9 text-center font-medium text-slate-600 border-transparent hover:border-slate-200 focus:border-blue-500 px-1" />
                                          </td>
                                          <td className="px-1 py-2">
                                              <Input type="number" min="1" value={item.quantity} onChange={e => { const items = [...editFormItems]; items[index].quantity = e.target.value; setEditFormItems(items); }} className="h-9 text-center font-black text-blue-600 border-transparent hover:border-slate-200 focus:border-blue-500 px-1" />
                                          </td>
                                          <td className="px-3 py-2 text-right">
                                              <button onClick={() => handleRemoveItemFromForm(index)} className="p-1.5 text-slate-400 hover:text-rose-500 hover:bg-rose-50 rounded-lg"><Trash2 className="h-4 w-4" /></button>
                                          </td>
                                      </tr>
                                  )})}
                              </tbody>
              ...
              <div className="shrink-0 pt-4 mt-2 border-t border-slate-100 flex gap-3">
                  <Button variant="outline" onClick={() => setIsFormEditModalOpen(false)} className="h-14 px-6 rounded-xl font-bold">Vazgeç</Button>
                  <Button onClick={handleSaveFormChanges} disabled={isSavingForm} className="flex-1 h-14 bg-indigo-600 hover:bg-indigo-700 text-white font-black text-base rounded-xl shadow-xl shadow-indigo-500/20">
