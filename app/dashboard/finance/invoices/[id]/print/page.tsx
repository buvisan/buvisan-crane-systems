...
export default function PrintInvoicePage() {
  ...
  if (!invoice) return <div className="flex h-screen items-center justify-center font-bold text-slate-500">Belge bulunamadı.</div>

  const isFatura = invoice.document_type === 'FATURA'

  return (
    <div className="min-h-screen bg-slate-100 flex flex-col items-center py-6 md:py-10 print:py-0 print:bg-white font-sans text-slate-800 overflow-x-hidden">
        ...
        <div className="w-full max-w-[210mm] flex flex-col sm:flex-row justify-between items-center gap-3 px-4 mb-4 md:mb-6 print:hidden">
            <button onClick={() => router.back()} className="w-full sm:w-auto flex items-center justify-center gap-2 px-4 py-2.5 bg-white border border-slate-200 rounded-lg md:rounded-xl text-sm font-bold text-slate-600 hover:bg-slate-50 shadow-sm">
                <ArrowLeft className="h-4 w-4" /> Geri Dön
            </button>
            <button onClick={() => window.print()} className="w-full sm:w-auto flex items-center justify-center gap-2 px-6 py-2.5 bg-blue-600 text-white rounded-lg md:rounded-xl text-sm font-bold hover:bg-blue-700 shadow-sm">
        ...
        <div className="w-full max-w-[100vw] overflow-x-auto pb-6 print:overflow-visible flex justify-center">
            <div className="w-[210mm] min-h-[297mm] bg-white shadow-xl print:shadow-none p-[15mm] relative box-border shrink-0">
