...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const [activeBg, setActiveBg] = useState("glass") 
  ...
  const getBackgroundClass = () => {
      ...
      if (activeBg === "solid") return "bg-muted"; 
  ...
  const menuGroups = [
    ...
    { title: "SATIN ALMA BİRİMİ", allowedRoles: ["satın","çelik kasa", "admin", "yönetim", "proje",], items: [ { href: "/dashboard/purchases", label: "Satın Alma Paneli", icon: ClipboardList }, { href: "/dashboard/purchase-history", label: "Sipariş Geçmişi", icon: History } ] },
    ...
    { title: "SAHA (ÜRETİM)", allowedRoles: ["imalat", "üretim", "saha", "montaj", "proje","satın","çelik kasa","otomasyon"], items: [ { href: "/dashboard/production-screen", label: "Üretim Ekranı", icon: HardHat } ] },
    { title: "DEPO & LOJİSTİK", allowedRoles: ["depo", "lojistik", "üretim", "satın", "mühendis", "proje","çelik kasa"], items: [ { href: "/dashboard/warehouse/products", label: "Depo Ürünleri (QR)", icon: Archive }, { href: "/dashboard/warehouse/entries", label: "Mal Kabul & İrsaliye", icon: ClipboardList }, { href: "/dashboard/warehouse/scanner", label: "QR Barkod Terminali", icon: ScanLine }, { href: "/dashboard/warehouse/logs", label: "Stok Hareket Analizi", icon: History } ] },
    ...
    { 
        ...
        allowedRoles: ["çelik kasa", "admin", "yönetim", "kurucu", "teknoloji", "kasa"], 
        items: [ 
                  ...
                  <div className="bg-muted/50 border border-border rounded-[1.5rem] p-4 flex flex-col gap-4">
                      <div className="grid grid-cols-2 md:grid-cols-6 gap-4">
                          <div className="space-y-2 col-span-2 md:col-span-3">
                              <Label className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Ürün Tanımı / Malzeme Adı</Label>
                              <Input placeholder="Örn: 6204 Rulman" value={orderItemForm.material_name} onChange={e=>setOrderItemForm({...orderItemForm, material_name: e.target.value})} className="font-bold border-border h-11 bg-background text-foreground" />
                          </div>
                          <div className="space-y-2 col-span-1 md:col-span-1">
                              <Label className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Mevcut Stok</Label>
