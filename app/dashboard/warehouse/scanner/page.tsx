...
import { ScanLine, Search, Loader2, PackageMinus, PackagePlus, ArrowRight, Image as ImageIcon, AlertCircle, CheckCircle2 } from "lucide-react"
...
export default function BarcodeScannerPage() {
  ...
  // Sayfa açıldığında otomatik olarak imleci barkod kutusuna odaklar (Okuyucu tabancalar için)
  useEffect(() => {
  ...
  const handleSearch = async (e?: React.FormEvent) => {
      ...
      const { data, error } = await supabase.from('warehouse_products').select('*').eq('product_code', barcode.trim()).single()
  ...
  const handleProcess = async () => {
      ...
      try {
          ...
          // Terminali sıfırla ve yeni barkod beklemeye geç
          setProduct(null)
          setAction(null)
          setBarcode("")
          inputRef.current?.focus()
    ...
    <div className="flex flex-col gap-8 max-w-[1000px] mx-auto w-full font-sans pb-10">
      ...
      <div className="flex items-center gap-5 bg-white/60 backdrop-blur-2xl border border-white/50 p-6 rounded-[2rem] shadow-sm">
        ...
        <div>
            ...
            <p className="text-slate-500 font-medium text-sm mt-1">Barkod okuyucu ile hızlı stok giriş/çıkış operasyon merkezi.</p>
        </div>
      </div>
      ...
      <div className="bg-slate-900 rounded-[2.5rem] p-8 md:p-12 shadow-2xl relative overflow-hidden">
          ...
          <form onSubmit={handleSearch} className="relative z-10 flex flex-col items-center max-w-xl mx-auto w-full gap-6">
              <div className="text-center space-y-2 mb-2">
                  ...
                  <p className="text-slate-400 font-medium text-sm">Ürünün üzerindeki QR kodu okuyucuyla taratın veya elle girin.</p>
              </div>
      ...
      {/* ÜRÜN BULUNDUĞUNDA AÇILAN PANEL */}
      {product && (
                  ...
                  <div className="flex flex-col gap-6 animate-in zoom-in-95 duration-300">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                          ...
                          <div className="md:col-span-2 space-y-3">
                              ...
                              <Input placeholder={action === 'OUT' ? "Örn: PRJ-2026 montajı için Kaya Ali'ye teslim edildi." : "Örn: Sahadan artan malzeme depoya iade edildi."} value={note} onChange={e => setNote(e.target.value)} className="h-16 rounded-2xl bg-slate-50 border-slate-200 font-bold px-5" />
                          </div>
                      </div>
