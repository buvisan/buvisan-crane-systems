...
export default function TrackingPersonnelPage() {
  ...
  const handleDelete = async (id: number) => {
    ...
    if (error) {
        // 🚀 GÜVENLİK UYARISI: Eğer personelin satışı varsa (Foreign Key hatası) uyar
  ...
  return (
    <div className="flex flex-col gap-8 max-w-[1400px] mx-auto w-full font-sans pb-10">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <Button onClick={() => { setIsAdding(!isAdding); setEditingId(null); setForm({ full_name: "", region: "", premium_limit: "", premium_percentage: "" }) }} className="h-24 px-8 bg-emerald-600 hover:bg-emerald-700 text-white text-base font-bold rounded-[2rem] shadow-lg shadow-emerald-500/30 transition-all flex flex-col items-center justify-center gap-2 shrink-0">
      ...
      {isAdding && (
          <div className="bg-white/80 backdrop-blur-2xl border border-white shadow-lg rounded-[2.5rem] p-8 animate-in fade-in slide-in-from-top-4">
              <h2 className="text-xl font-black text-slate-800 mb-6">{editingId ? "Personel Bilgilerini Güncelle" : "Yeni Personel Kaydı"}</h2>
          ...
          <table className="w-full text-left border-collapse">
              <thead className="bg-emerald-900 text-white">
                  <tr>
                      ...
                      <th className="px-6 py-5 text-xs font-bold uppercase tracking-widest text-right">İşlem</th>
                  </tr>
              </thead>
              <tbody className="divide-y divide-emerald-100">
                  {personnel.map((p) => {
                      ...
                      <tr key={p.id} className="hover:bg-emerald-50/50 transition-colors bg-emerald-50/20">
                          ...
                          <td className="px-6 py-4"><span className="flex items-center gap-1 text-xs font-bold text-emerald-700 bg-white border border-emerald-200 px-2 py-1 rounded w-max"><MapPin className="h-3 w-3" /> {p.region}</span></td>
                          <td className="px-6 py-4 text-sm font-bold text-emerald-800 tabular-nums">{p.premium_limit.toLocaleString('tr-TR', { style: 'currency', currency: 'TRY' })}</td>
                          <td className="px-6 py-4 text-sm font-bold text-emerald-800 text-center">%{p.premium_percentage}</td>
                          <td className="px-6 py-4 text-sm font-black text-slate-800 tabular-nums">{p.totalSales.toLocaleString('tr-TR', { style: 'currency', currency: 'TRY' })}</td>
                          <td className="px-6 py-4">
                              <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-sm font-black tabular-nums shadow-sm ${limitPassed && p.earnedPremium > 0 ? 'bg-emerald-500 text-white shadow-emerald-500/30' : 'bg-white border border-slate-200 text-slate-400'}`}>
                                  <Award className="h-4 w-4" /> {p.earnedPremium.toLocaleString('tr-TR', { style: 'currency', currency: 'TRY' })}
                              </span>
                          </td>
