...
export default function TrackingSalesPage() {
        ...
        const grouped = salesData.reduce((acc: any, curr: any) => {
            ...
            if (!acc[key]) {
                acc[key] = {
                    ...
                    all_ids: [curr.id],
                    items: [curr],
                    combined_machines: `${curr.tracking_products?.brand} (${curr.tracking_products?.model})`,
                    total_cart_amount: curr.total_amount,
                    total_cart_quantity: curr.quantity
                }
            } else {
                acc[key].all_ids.push(curr.id)
                acc[key].items.push(curr)
                acc[key].combined_machines += ` / ${curr.tracking_products?.brand} (${curr.tracking_products?.model})`
                acc[key].total_cart_amount += curr.total_amount
                acc[key].total_cart_quantity += curr.quantity
            }
            return acc
        }, {})
  ...
  const openEdit = (saleGroup: any) => {
      const rate = rates[currency] || 1
      setForm({
          sale_date: saleGroup.sale_date, personnel_id: saleGroup.personnel_id.toString(), customer_name: saleGroup.customer_name, customer_location: saleGroup.customer_location
      })
      
      const loadedItems = saleGroup.items.map((i: any) => ({
          product_id: i.product_id?.toString() || "", 
          brand: i.tracking_products?.brand || "", 
          model: i.tracking_products?.model || "",
          list_price: i.list_price * rate, 
          discount_rate: i.discount_rate, 
          sale_price: i.sale_price * rate, 
          quantity: i.quantity, 
          total_amount: i.total_amount * rate
      }))
  ...
  const filteredSales = sales.filter(s => 
      ...
      s.combined_machines?.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    <div className="flex flex-col gap-6 md:gap-8 max-w-[1600px] mx-auto w-full font-sans pb-10">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full md:w-auto">
            ...
            <Button onClick={() => { setIsAdding(!isAdding); setEditingIds(null); setForm({ sale_date: new Date().toISOString().split('T')[0], personnel_id: "", customer_name: "", customer_location: "" }); setItems([{ product_id: "", brand: "", model: "", list_price: 0, discount_rate: 0, sale_price: 0, quantity: 1, total_amount: 0 }]); }} className="h-12 md:h-14 px-6 md:px-8 bg-amber-500 hover:bg-amber-600 text-white text-sm md:text-base font-bold rounded-xl md:rounded-[2rem] shadow-lg shadow-amber-500/30 transition-all flex items-center justify-center gap-2 shrink-0">
      ...
      {isAdding && (
          <div className="bg-white/80 backdrop-blur-2xl border border-white shadow-xl shadow-amber-500/5 rounded-[1.5rem] md:rounded-[2.5rem] p-5 md:p-8 animate-in fade-in slide-in-from-top-4">
              <div className="flex items-center gap-2 md:gap-3 mb-5 md:mb-6 border-b border-slate-100 pb-4">
                  ...
                  <h2 className="text-lg md:text-xl font-black text-slate-800">{editingIds ? "Satış Kaydını Düzenle" : "Çoklu Satış Formu"}</h2>
              </div>
              ...
              <form onSubmit={handleSave} className="flex flex-col gap-6 md:gap-8">
                  ...
                  <div className="flex flex-col-reverse md:flex-row items-end justify-between gap-4 border-t border-slate-100 pt-6">
                      <div className="flex gap-2 w-full md:w-auto">
                          <Button type="button" variant="ghost" onClick={() => { setIsAdding(false); setEditingIds(null); }} className="h-12 md:h-14 px-6 rounded-xl md:rounded-2xl font-bold w-full md:w-auto bg-slate-100 hover:bg-slate-200">İptal Et</Button>
                          <Button type="submit" disabled={saving} className="h-12 md:h-14 px-6 md:px-10 rounded-xl md:rounded-2xl bg-amber-500 hover:bg-amber-600 text-white font-black shadow-xl shadow-amber-500/20 text-sm md:text-lg w-full md:w-auto">
                              {saving ? <Loader2 className="mr-2 h-4 w-4 md:h-5 md:w-5 animate-spin" /> : (editingIds ? "Sepeti Güncelle" : "Satışı Kaydet")}
                          </Button>
                      </div>
              ...
              <table className="w-full text-left border-collapse whitespace-nowrap min-w-[1000px]">
                  <thead className="bg-[#4b5e40] text-white">
                      <tr>
                          ...
                          <th className="px-4 md:px-6 py-4 md:py-5 text-[9px] md:text-[11px] font-black uppercase tracking-widest">Marka & Modeller</th>
                          <th className="px-4 md:px-6 py-4 md:py-5 text-[9px] md:text-[11px] font-black uppercase tracking-widest text-center">Toplam Adet</th>
                          <th className="px-4 md:px-6 py-4 md:py-5 text-[9px] md:text-[11px] font-black uppercase tracking-widest bg-[#3d4d34]">Sepet Tutarı</th>
                  ...
                  <tbody className="divide-y divide-[#e1e6de] bg-[#ebf0e8]/50">
                      {filteredSales.map((sale) => (
                          <tr key={sale.id} className="hover:bg-[#f1f4ef] transition-colors">
                              ...
                              <td className="px-4 md:px-6 py-3 md:py-4 text-xs md:text-sm font-bold text-slate-800 max-w-[250px] truncate" title={sale.combined_machines}>
                                  {sale.combined_machines}
                              </td>
                              <td className="px-4 md:px-6 py-3 md:py-4 text-xs md:text-sm font-black text-blue-600 text-center bg-blue-50/50">{sale.total_cart_quantity}</td>
                              <td className="px-4 md:px-6 py-3 md:py-4 text-xs md:text-sm font-black text-[#2e4024] bg-[#dbe6d5] tabular-nums shadow-inner">{formatPrice(sale.total_cart_amount)}</td>
                              <td className="px-4 md:px-6 py-3 md:py-4 text-right flex justify-end gap-1.5 md:gap-2">
                                  ...
                                  <button onClick={() => handleDelete(sale.all_ids)} className="p-1.5 md:p-2 text-slate-400 hover:text-rose-500 hover:bg-rose-50 rounded-lg md:rounded-xl transition-colors"><Trash2 className="h-4 w-4 md:h-5 md:w-5" /></button>
                              </td>
                          </tr>
                      ))}
                      {filteredSales.length === 0 && !loading && (
                          <tr><td colSpan={7} className="py-16 md:py-20 text-center bg-white"><div className="flex flex-col items-center gap-3"><div className="bg-amber-50 p-4 md:p-5 rounded-full shadow-sm"><Receipt className="h-8 w-8 md:h-10 md:w-10 text-amber-300" /></div><p className="text-base md:text-lg font-bold text-amber-600">Satış kaydı bulunmuyor.</p></div></td></tr>
                      )}
                  </tbody>
              </table>
          </div>
