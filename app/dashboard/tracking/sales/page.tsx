...
export default function TrackingSalesPage() {
  ...
  const openEdit = (sale: any) => {
      ...
      setIsAdding(true)
      window.scrollTo({ top: 0, behavior: 'smooth' }) // Mobilde forma kaysın
  }
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (!finalProductId) {
        const { data: newProd, error: prodErr } = await supabase.from('tracking_products').insert([{
            ...
            price: Number(form.list_price) / rate, 
            stock: 1
        }]).select().single()
    ...
    <div className="flex flex-col gap-6 md:gap-8 max-w-[1600px] mx-auto w-full font-sans pb-10">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full md:w-auto">
            ...
            <Button onClick={() => { setIsAdding(!isAdding); setEditingId(null); setForm({ sale_date: new Date().toISOString().split('T')[0], personnel_id: "", customer_name: "", customer_location: "", product_id: "", brand: "", model: "", list_price: 0, discount_rate: 0, sale_price: 0, quantity: 1, total_amount: 0 }) }} className="h-12 md:h-14 px-6 md:px-8 bg-amber-500 hover:bg-amber-600 text-white text-sm md:text-base font-bold rounded-xl md:rounded-[2rem] shadow-lg shadow-amber-500/30 transition-all flex items-center justify-center gap-2 shrink-0">
                <PlusCircle className="h-4 w-4 md:h-5 md:w-5" /> Yeni Satış Gir
            </Button>
        </div>
      </div>

      {/* 🚀 FORM ALANI (Mobilde Alt Alta Düzenlendi) */}
      {isAdding && (
          <div className="bg-white/80 backdrop-blur-2xl border border-white shadow-xl shadow-amber-500/5 rounded-[1.5rem] md:rounded-[2.5rem] p-5 md:p-8 animate-in fade-in slide-in-from-top-4">
              <div className="flex items-center gap-2 md:gap-3 mb-5 md:mb-6 border-b border-slate-100 pb-4">
                  ...
                  <h2 className="text-lg md:text-xl font-black text-slate-800">{editingId ? "Satış Kaydını Düzenle" : "Akıllı Satış Formu"}</h2>
              </div>
              ...
              <form onSubmit={handleSave} className="grid grid-cols-1 md:grid-cols-12 gap-5 md:gap-6">
                  ...
                  <div className="md:col-span-8 grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div className="space-y-1.5 md:space-y-2">
                          ...
                          <Input required type="date" value={form.sale_date} onChange={e => setForm({...form, sale_date: e.target.value})} className="h-12 rounded-xl bg-slate-50 border-slate-200 text-sm" />
                      </div>
                      <div className="space-y-1.5 md:space-y-2">
                          <Label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Satış Personeli</Label>
                          <select required value={form.personnel_id} onChange={e => setForm({...form, personnel_id: e.target.value})} className="w-full h-12 px-3 md:px-4 rounded-xl bg-slate-50 border border-slate-200 text-xs md:text-sm font-bold text-slate-700 outline-none focus:ring-2 focus:ring-amber-500">
                      ...
                      <div className="space-y-1.5 md:space-y-2">
                          ...
                          <Input required placeholder="Örn: Kaya Ali Tosun" value={form.customer_name} onChange={e => setForm({...form, customer_name: e.target.value})} className="h-12 rounded-xl bg-slate-50 border-slate-200 text-sm" />
                      </div>
                      <div className="space-y-1.5 md:space-y-2">
                          <Label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Müşteri Konumu (İl)</Label>
                          <Input required placeholder="Örn: Bursa" value={form.customer_location} onChange={e => setForm({...form, customer_location: e.target.value})} className="h-12 rounded-xl bg-slate-50 border-slate-200 text-sm" />
                      </div>
                      
                      <div className="sm:col-span-2 grid grid-cols-1 sm:grid-cols-3 gap-3 md:gap-4 bg-amber-50/50 p-4 rounded-2xl border border-amber-100">
                          <div className="space-y-1.5 md:space-y-2 relative sm:col-span-1">
                              ...
                              {isProductDropdownOpen && form.brand && (
                                  <div className="absolute z-50 w-full sm:w-[250%] mt-1 bg-white rounded-xl border border-slate-200 shadow-xl max-h-48 overflow-y-auto p-1">
                                      ...
                                      {products.filter(p => p.brand.toLowerCase().includes(form.brand.toLowerCase())).length === 0 && (
                                          <div className="px-3 py-2 text-[10px] md:text-[11px] font-bold text-emerald-600 text-center bg-emerald-50 rounded-lg">✨ Yeni makine eklenecek</div>
                                      )}
                                  </div>
                              )}
                          </div>
                          <div className="space-y-1.5 md:space-y-2">
                              ...
                              <Input placeholder="Örn: 2026" value={form.model} onChange={e => setForm({...form, model: e.target.value, product_id: ""})} className="h-12 rounded-xl bg-white border-amber-200 focus:ring-amber-500 font-bold text-sm" />
                          </div>
                          <div className="space-y-1.5 md:space-y-2">
                              <Label className="text-[10px] font-black text-rose-500 uppercase tracking-widest bg-rose-50 px-1.5 md:px-2 py-0.5 rounded">Liste Fiyatı ({currency})</Label>
                              <Input type="number" value={form.list_price || ""} onChange={e => setForm({...form, list_price: Number(e.target.value), product_id: ""})} className="h-12 rounded-xl bg-white border-amber-200 focus:ring-amber-500 font-bold text-amber-700 text-sm" />
                          </div>
                      </div>
                  </div>

                  {/* SAĞ TARAF: HESAPLAMALAR VE KAYDET */}
                  <div className="md:col-span-4 flex flex-col gap-4 bg-slate-50 p-4 md:p-5 rounded-2xl border border-slate-100">
                      <div className="grid grid-cols-2 gap-3 md:gap-4">
                          <div className="space-y-1.5 md:space-y-2">
                              <Label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">İskonto (%)</Label>
                              <Input type="number" step="0.01" value={form.discount_rate} onChange={e => setForm({...form, discount_rate: Number(e.target.value)})} className="h-12 rounded-xl bg-white border-slate-200 font-bold text-sm" />
                          </div>
                          <div className="space-y-1.5 md:space-y-2">
                              <Label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Adet</Label>
                              <Input type="number" min="1" value={form.quantity} onChange={e => setForm({...form, quantity: Number(e.target.value)})} className="h-12 rounded-xl bg-white border-slate-200 font-bold text-blue-600 text-sm" />
                          </div>
                      </div>
                      <div className="mt-auto space-y-2 md:space-y-3 pt-3 md:pt-4 border-t border-slate-200">
                          <div className="flex justify-between items-center">
                              <span className="text-[10px] md:text-xs font-bold text-slate-500">Birim Satış Fiyatı:</span>
                          ...
                          <div className="flex justify-between items-end bg-emerald-100 p-2.5 md:p-3 rounded-xl border border-emerald-200">
                              <span className="text-[10px] md:text-xs font-black text-emerald-800 uppercase tracking-widest">TOPLAM TUTAR</span>
                  ...
                  <div className="md:col-span-12 flex flex-col-reverse sm:flex-row justify-end gap-3 mt-2 md:mt-4">
                      <Button type="button" variant="ghost" onClick={() => { setIsAdding(false); setEditingId(null); }} className="h-12 md:h-14 px-6 md:px-8 rounded-xl md:rounded-2xl font-bold w-full sm:w-auto">İptal Et</Button>
                      <Button type="submit" disabled={saving} className="h-12 md:h-14 px-6 md:px-10 rounded-xl md:rounded-2xl bg-amber-500 hover:bg-amber-600 text-white font-black shadow-xl shadow-amber-500/20 text-sm md:text-lg w-full sm:w-auto">
                          {saving ? <Loader2 className="mr-2 h-4 w-4 md:h-5 md:w-5 animate-spin" /> : (editingId ? "Satışı Güncelle" : "Satışı Kaydet")}
                      </Button>
                  </div>
              </form>
          </div>
              ...
              <table className="w-full text-left border-collapse whitespace-nowrap min-w-[1000px]">
                  <thead className="bg-[#4b5e40] text-white">
                      <tr>
                          ...
                          <th className="px-4 md:px-6 py-4 md:py-5 text-[9px] md:text-[11px] font-black uppercase tracking-widest">Müşteri Adı</th>
                          <th className="px-4 md:px-6 py-4 md:py-5 text-[9px] md:text-[11px] font-black uppercase tracking-widest">Müşteri Konumu</th>
                          <th className="px-4 md:px-6 py-4 md:py-5 text-[9px] md:text-[11px] font-black uppercase tracking-widest">Marka</th>
                          <th className="px-4 md:px-6 py-4 md:py-5 text-[9px] md:text-[11px] font-black uppercase tracking-widest">Model</th>
                          ...
                          <th className="px-4 md:px-6 py-4 md:py-5 text-[9px] md:text-[11px] font-black uppercase tracking-widest bg-[#3d4d34]">Toplam Tutar</th>
                  ...
                  <tbody className="divide-y divide-[#e1e6de] bg-[#ebf0e8]/50">
                      {filteredSales.map((sale) => (
                          <tr key={sale.id} className="hover:bg-[#f1f4ef] transition-colors">
                              ...
                              <td className="px-4 md:px-6 py-3 md:py-4 text-xs md:text-sm font-bold text-slate-700">{sale.customer_name}</td>
                              <td className="px-4 md:px-6 py-3 md:py-4 text-xs md:text-sm font-medium text-slate-600">{sale.customer_location}</td>
                              <td className="px-4 md:px-6 py-3 md:py-4 text-xs md:text-sm font-bold text-slate-800 truncate max-w-[150px]">{sale.tracking_products?.brand}</td>
                              <td className="px-4 md:px-6 py-3 md:py-4 text-xs md:text-sm font-bold text-slate-600">{sale.tracking_products?.model}</td>
                      ...
                      {filteredSales.length === 0 && !loading && (
                          <tr><td colSpan={12} className="py-16 md:py-20 text-center bg-white"><div className="flex flex-col items-center gap-3"><div className="bg-amber-50 p-4 md:p-5 rounded-full shadow-sm"><Receipt className="h-8 w-8 md:h-10 md:w-10 text-amber-300" /></div><p className="text-base md:text-lg font-bold text-amber-600">Satış kaydı bulunmuyor.</p></div></td></tr>
                      )}
                  </tbody>
              </table>
          </div>
